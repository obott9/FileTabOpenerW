...
- **タブグループ管理** - タブグループの作成、名前変更、コピー、削除、並べ替え。コピー名は `"{ベース名} {N}"` パターン: "作業" → "作業 1" → "作業 2"
- **ワンクリックオープン** - タブグループ内の全フォルダを1つのエクスプローラーウィンドウでタブとして開く
- **デュアルレイアウト** - コンパクトレイアウト（デフォルト、タブボタンバー）とサイドバーレイアウト（ListView＋詳細パネル）。サイドバーレイアウトは macOS SwiftUI 版から移植
...
- **ネットワークパス対応** - UNC パス (`\\server\share`) はオフラインでも登録可能。認証が必要な場合はエクスプローラーが認証ダイアログを表示
...
1. [CMake Tools](https://marketplace.visualstudio.com/items?itemName=ms-vscode.cmake-tools) 拡張機能をインストール
2. ステータスバーで **CMake: [Release]** を選択
3. ステータスバーでビルドターゲット **[FileTabOpenerW]** を選択
4. ステータスバーの **ビルド** ボタンをクリック（または `Ctrl+Shift+B`）
...
1. **UI Automation (UIA)** - 主要な方法。Windows UI Automation API を使用してエクスプローラーの「新しいタブ」ボタンとアドレスバーを検出し、プログラムでタブを作成してパスに移動します。Enter キーは PostMessage でウィンドウに直接送信（グローバルキーストロークではない）。
2. **SendInput** - フォールバック。OS レベルのキーストローク（Ctrl+T、Ctrl+L、パス入力、Enter）をシミュレートします。フォーカスやタイミングの問題で信頼性が低い場合があります。
3. **個別ウィンドウ** - 最終手段。各フォルダを個別のエクスプローラーウィンドウで開きます。

### ネットワークパス

UNC パス (`\\server\share`) に対応しています。ネットワーク共有は認証が必要な場合があるため、UNC パスは通常のディレクトリ存在確認をスキップし、そのままエクスプローラーに渡されます。

認証が必要な UNC パスの場合：
1. エクスプローラーが Windows セキュリティの資格情報ダイアログを表示
2. アプリは認証を待たずに残りのタブを開き続ける
3. タブは一時的に「PC」をプレースホルダーとして表示
4. ユーザーが認証後、エクスプローラーが実際のネットワーク共有に移動

ユーザーが認証ダイアログをキャンセルした場合、タブは「PC」のまま残ります。

### パフォーマンスに関する注意

Windows エクスプローラーにはタブ操作用の公開 API がありません。すべての方法は UI Automation またはキーストロークシミュレーションに依存しており、UI が応答するまでタブごとに待機が必要です。多数のタブを開く場合、予想より遅くなることがあります。

> **重要（SendInput フォールバック）：** タブを開いている間はキーボードやマウスを使用しないでください。SendInput メソッドは OS レベルのキーストロークシミュレーションを使用するため、操作中の入力が自動化に干渉する可能性があります。UIA メソッドは主にターゲット指定の UI Automation と PostMessage を使用しますが（グローバルキーストロークなし）、UIA 操作が失敗した場合はキーボード入力にフォールバックすることがあります。
