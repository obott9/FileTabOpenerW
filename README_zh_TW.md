...
- **分頁群組管理** - 建立、重新命名、複製、刪除及排序分頁群組。複製命名遵循 `"{基礎名} {N}"` 模式：「工作」→「工作 1」→「工作 2」
- **一鍵開啟** - 將分頁群組中的所有資料夾在一個檔案總管視窗中以分頁方式開啟
- **雙佈局** - 精簡佈局（預設，分頁按鈕列）與側邊欄佈局（ListView + 詳細面板）；側邊欄佈局移植自 macOS SwiftUI 版
...
- **網路路徑支援** - UNC 路徑（`\\server\share`）即使離線也可註冊；需要驗證時由檔案總管彈出驗證對話框
...
1. **UI Automation (UIA)** - 主要方法。使用 Windows UI Automation API 尋找檔案總管的「新增分頁」按鈕和網址列，以程式方式建立分頁並導覽至各路徑。Enter 鍵透過 PostMessage 直接傳送至視窗（非全域按鍵）。
2. **SendInput** - 備用方法。使用作業系統層級的按鍵模擬（Ctrl+T、Ctrl+L、路徑輸入、Enter）。由於焦點和時序問題，可靠性較低。
3. **個別視窗** - 最終手段。以個別檔案總管視窗開啟每個資料夾。

### 網路路徑

支援 UNC 路徑（`\\server\share`）。由於網路共享可能需要驗證，UNC 路徑會跳過一般的目錄存在性驗證，直接傳遞給檔案總管。

當 UNC 路徑需要驗證時：
1. 檔案總管顯示 Windows 安全性認證對話框
2. 應用程式不等待驗證，繼續開啟剩餘分頁
3. 分頁暫時顯示「本機」作為佔位符
4. 使用者完成驗證後，檔案總管導覽至實際的網路共享

如果使用者取消驗證對話框，分頁將保留在「本機」。

### 效能說明

Windows 檔案總管沒有提供分頁操作的公開 API。所有方法都依賴 UI Automation 或按鍵模擬，每個分頁之間需要等待 UI 回應。開啟多個分頁時可能比預期更慢。

> **重要（SendInput 備用方法）：** 開啟分頁期間請勿使用鍵盤或滑鼠。SendInput 方法使用作業系統層級的按鍵模擬，操作期間的輸入可能干擾自動化過程。UIA 方法主要使用定向 UI Automation 和 PostMessage（無全域按鍵），但當 UIA 操作失敗時可能回退到鍵盤輸入。
